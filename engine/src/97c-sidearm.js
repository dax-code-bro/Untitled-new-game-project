...
Object.assign(SERVICE_KINDS, {
  ...
  model5: sideSpec({
    ammoKind: 'full',
    /* 205 mm to the muzzle, not 242. Overall this came out at 348 mm
       against a class ceiling of 340 and a real .50 hand cannon's 269 --
       it was not marginally long, it was a third longer than the gun it
       is. Restoring the grip that a duplicate key had wiped bought 3 mm
       of that; the other 37 were simply the barrel. The rear is where a
       hand goes and does not move; everything forward of the frame
       comes back. */
    muzzle: 0.205,
    ...
    /* e below 4.4, because at or above it svcDetails stamps six
       ...
       P226 and the G18. */
    rec: { rear: -0.060, front: 0.170, up: 0.0186, down: 0.0150, w: 0.0150, e: 4.2 },
    port: { x0: 0.040, x1: 0.084, up: 0.0130, down: 0.0020 },
    grip: { x: -0.044, y: -0.0190, len: 0.108, rake: 0.26,
      deep: 1.14, wide: 1.10, e: 1.20, checkN: [6, 8], checkH: 0.0012 },
    mag: { x: -0.044, y: -0.0210, len: 0.100, w: 0.0110, d: 0.0130, r: 0.028 },
    sight: { y: 0.0230, frontX: 0.162, rearX: -0.046 },
    ...
    rib: { x0: -0.030, x1: 0.164, hw: 0.0078, vent: true },
    comp: { x0: 0.176, x1: 0.198, n: 3, w: 0.0036, hw: 0.0026 },
...
/* IN, NOT OUT. The spur is drawn 15 mm behind its own pin, so at
  ...
  webley: sideSpec({
    ...
    // Bird's head butt: deep front to back, narrow across, coarse.
    grip: { x: -0.040, y: -0.0170, len: 0.098, rake: 0.46,
      deep: 1.10, wide: 0.86, e: 0.80, checkN: [5, 9], checkH: 0.0013 },
    mag: null,
  ...
  mauser: sideSpec({
    ...
    // Genuinely circular in section, which is why it is called that.
    grip: { x: -0.048, y: -0.0170, len: 0.086, rake: 0.16,
      deep: 0.94, wide: 1.06, e: 0.62, check: false },
  ...
  p226: sideSpec({
    ...
    grip: { x: -0.034, y: -0.0165, len: 0.098, rake: 0.24,
      deep: 0.92, wide: 1.16, e: 1.35, checkH: 0.0007 },
  ...
  g18: sideSpec({
    ...
    grip: { x: -0.032, y: -0.0165, len: 0.096, rake: 0.22,
      deep: 0.90, wide: 1.20, e: 1.45, checkN: [7, 10], checkH: 0.0006 },
...
Object.assign(SERVICE_KINDS, {
  ...
  killstreak: boltSpec({
    /* 0.680, not 0.820: the can takes the muzzle out to 0.840 and the
       barrel stops short of it. Written as a second `muzzle` further
       down the block it was silently winning over this one, which is
       the same JavaScript footgun that wiped five grips. One key. */
    muzzle: 0.680,
