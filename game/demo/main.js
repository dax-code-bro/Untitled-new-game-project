...
function buildSkinnedBody(kind, preset) {
  ...
  if (preset.meshyBody && MOLLY) {
    ...
    if (!MOLLY.bound) {
      ...
      // NON-DESTRUCTIVE bind — WEIGHTS ONLY, geometry left exactly as the GLB
      // ships it. During the "stretching" era the FULL mesh loaded on every
      // device; the cure for the stretch is the weight CLAMP below (soft joints,
      // no cross-limb bleed), NOT cutting triangles. Every geometry-rebuild pass
      // (bridge-cut, stretch-delete, island-remove, winding-flip) is GONE —
      // rebuilding the mesh is exactly what the creator's iPhone failed to load.
...
(function loadAssets() {
  ...
  loader.load('./assets/molotov4.glb', (g) => {
...
const BUILD = 71;   // bump with each demo update — shown on the badge so staleness is visible
