...
function bindGeoToBones(geo, B, segs, rigid = []) {
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    if (rz2) { SI.push(B.indexOf(rz2.b), 0, 0, 0); SW.push(1, 0, 0, 0); grp.push(0); dom.push(B.indexOf(rz2.b)); continue; }
    ...
    if (rz3) {                                   // boundary band: half local bone, half the rigid anchor
      ...
      SW.push(0.5, 0.5, 0, 0);
      grp.push(G(s1.ch)); dom.push(B.indexOf(s1.b));
      continue;
    }
    SI.push(B.indexOf(s1.b), s2 ? B.indexOf(s2.b) : 0, 0, 0);
    SW.push(w1, w2, 0, 0);
  ...
  // remember each vertex's PRE-smoothing limb group + dominant bone, so smoothing
  // can be prevented from bleeding one limb's influence onto another afterward
  geo.userData.limbGroup = grp;
  geo.userData.domBone = dom;
}

// GROUP CLAMP — after smoothing, forbid any vertex from being weighted to a bone
// in a DIFFERENT limb group than its proximity binding assigned. Smoothing stays
// soft WITHIN a group (elbow, knee, deltoid-to-chest inside the torso) but can no
// longer bleed arm weight onto the chest centre or finger weight onto a thigh —
// which is what tears a rigid patch loose from the body. boneGroup[i] maps a bone
// index to 0/1/2; the pre-smoothing dominant bone is the safe fallback.
function clampWeightsToGroups(geo, boneGroup) {
  const grp = geo.userData.limbGroup, dom = geo.userData.domBone;
  if (!grp) return;
  const si = geo.attributes.skinIndex, sw = geo.attributes.skinWeight;
  let fixed = 0;
  for (let i = 0; i < si.count; i++) {
    const g = grp[i];
    let b0 = si.getX(i), w0 = sw.getX(i), b1 = si.getY(i), w1 = sw.getY(i);
    let changed = false;
    if (w0 > 0 && boneGroup[b0] !== g) { w0 = 0; changed = true; }
    if (w1 > 0 && boneGroup[b1] !== g) { w1 = 0; changed = true; }
    if (!changed) continue;
    const tot = w0 + w1;
    if (tot > 1e-5) { sw.setX(i, w0 / tot); sw.setY(i, w1 / tot); }
    else { si.setX(i, dom[i]); si.setY(i, 0); sw.setX(i, 1); sw.setY(i, 0); }  // fully drifted → snap home
    fixed++;
  }
  si.needsUpdate = true; sw.needsUpdate = true;
  console.log('[demo] group clamp corrected', fixed, 'cross-limb-weighted vertices');
}
...
function cutArmLegBridges(geo) {
  ...
  const zone = (i) => {
    ...
    if (y > 1.0 || y < 0.45) return 0;
    return x > 0.222 ? 1 : x < 0.218 ? 2 : 0;   // 1 = hand-space, 2 = body/leg (tight seam)
  };
  let cut = 0;
  for (let t = 0; t < pos.count; t += 3) {
    const z1 = zone(t), z2 = zone(t + 1), z3 = zone(t + 2);
    const hasHand = z1 === 1 || z2 === 1 || z3 === 1;
    const hasLeg = z1 === 2 || z2 === 2 || z3 === 2;
    if (hasHand && hasLeg) { cut++; continue; }
    for (let k = 0; k < 3; k++) {
  ...
  console.log('[demo] cut', cut, 'hand-leg bridge triangles');
...
// remove orphaned scraps left behind by the scissors: connected components
// with too few triangles to be a real body part
function removeSmallIslands(geo, minTris = 300) {
  const pos = geo.attributes.position;
  const n = pos.count;
  ...
  for (let i = 0; i < n; i++) {
    ...
    let g = seen.get(k);
    if (g === undefined) { seen.set(k, i); g = i; }
    rep[i] = g;
  }
  const parent = new Array(n).fill(0).map((_, i) => i);
  const find = (a) => { while (parent[a] !== a) { parent[a] = parent[parent[a]]; a = parent[a]; } return a; };
  const uni = (a, b) => { parent[find(a)] = find(b); };
  for (let t = 0; t < n; t += 3) { uni(rep[t], rep[t + 1]); uni(rep[t + 1], rep[t + 2]); }
  const triCount = new Map();
  for (let t = 0; t < n; t += 3) {
    const r = find(rep[t]);
    triCount.set(r, (triCount.get(r) || 0) + 1);
  }
  const uv = geo.attributes.uv, nor = geo.attributes.normal, si = geo.attributes.skinIndex, sw = geo.attributes.skinWeight;
  const P = [], U = [], N = [], S = [], W = [];
  let cut = 0;
  for (let t = 0; t < n; t += 3) {
    if (triCount.get(find(rep[t])) < minTris) { cut++; continue; }
    for (let k = 0; k < 3; k++) {
      P.push(pos.getX(t + k), pos.getY(t + k), pos.getZ(t + k));
      N.push(nor.getX(t + k), nor.getY(t + k), nor.getZ(t + k));
      U.push(uv.getX(t + k), uv.getY(t + k));
      if (si) { S.push(si.getX(t + k), si.getY(t + k), 0, 0); W.push(sw.getX(t + k), sw.getY(t + k), 0, 0); }
    }
  }
  console.log('[demo] island cleanup removed', cut, 'orphan triangles');
  const out = new THREE.BufferGeometry();
  out.setAttribute('position', new THREE.Float32BufferAttribute(P, 3));
  out.setAttribute('normal', new THREE.Float32BufferAttribute(N, 3));
  out.setAttribute('uv', new THREE.Float32BufferAttribute(U, 2));
  if (si) {
    out.setAttribute('skinIndex', new THREE.Uint16BufferAttribute(S, 4));
    out.setAttribute('skinWeight', new THREE.Float32BufferAttribute(W, 4));
  }
  return out;
}

// DEFINITIVE skinning cleanup — pose the bound skeleton through the real
// animation extremes and delete every triangle that BOTH stretches past
// `ratioThr`x its rest size AND ends up physically long (> `absThr` metres).
// Cause-agnostic: any fused-across-body geometry (the #1 artifact in AI-
// generated character meshes — fingertips welded to a thigh, a coat, a sleeve)
// can only reveal itself as a triangle that tears when the limbs move, so this
// catches every one wherever it hides, while sparing legit joint creases (which
// stay physically small even when their ratio spikes). poseFns each mutate bone
// rotations; the rest pose is saved and restored around the analysis.
function deleteStretchTriangles(geo, bones, poseFns, ratioThr = 2.2, absThr = 0.09) {
  const root = bones[0];
  const saved = bones.map(b => b.rotation.clone());
  root.updateMatrixWorld(true);
  const restInv = bones.map(b => b.matrixWorld.clone().invert());
  const pos = geo.attributes.position, si = geo.attributes.skinIndex, sw = geo.attributes.skinWeight;
  const n = pos.count;
  const rest = new Float32Array(n * 3);
  for (let i = 0; i < n; i++) { rest[i * 3] = pos.getX(i); rest[i * 3 + 1] = pos.getY(i); rest[i * 3 + 2] = pos.getZ(i); }
  const def = new Float32Array(n * 3);
  const bad = new Uint8Array(n / 3);
  const skinMat = bones.map(() => new THREE.Matrix4());
  const base = new THREE.Vector3(), tmp = new THREE.Vector3(), acc = new THREE.Vector3();
  for (const poseFn of poseFns) {
    bones.forEach((b, i) => b.rotation.copy(saved[i]));   // reset to rest
    poseFn();                                             // strike the test pose
    root.updateMatrixWorld(true);
    for (let i = 0; i < bones.length; i++) skinMat[i].multiplyMatrices(bones[i].matrixWorld, restInv[i]);
    for (let v = 0; v < n; v++) {                         // linear-blend-skin every vertex
      base.set(rest[v * 3], rest[v * 3 + 1], rest[v * 3 + 2]);
      acc.set(0, 0, 0);
      const b0 = si.getX(v), w0 = sw.getX(v), b1 = si.getY(v), w1 = sw.getY(v);
      if (w0 > 0) { tmp.copy(base).applyMatrix4(skinMat[b0]); acc.addScaledVector(tmp, w0); }
      if (w1 > 0) { tmp.copy(base).applyMatrix4(skinMat[b1]); acc.addScaledVector(tmp, w1); }
      const tw = w0 + w1;
      if (tw > 0 && Math.abs(tw - 1) > 1e-4) acc.multiplyScalar(1 / tw);
      def[v * 3] = acc.x; def[v * 3 + 1] = acc.y; def[v * 3 + 2] = acc.z;
    }
    for (let t = 0; t < n; t += 3) {                      // flag torn triangles
      if (bad[t / 3]) continue;
      for (let e = 0; e < 3; e++) {
        const a = t + e, b = t + ((e + 1) % 3);
        const rl = Math.hypot(rest[a * 3] - rest[b * 3], rest[a * 3 + 1] - rest[b * 3 + 1], rest[a * 3 + 2] - rest[b * 3 + 2]);
        const dl = Math.hypot(def[a * 3] - def[b * 3], def[a * 3 + 1] - def[b * 3 + 1], def[a * 3 + 2] - def[b * 3 + 2]);
        const ratio = rl > 1e-4 ? dl / rl : (dl > 0.05 ? 999 : 1);
        if (ratio > ratioThr && dl > absThr) { bad[t / 3] = 1; break; }
      }
    }
  }
  bones.forEach((b, i) => b.rotation.copy(saved[i]));     // restore rest pose for the real bind
  root.updateMatrixWorld(true);
  const uv = geo.attributes.uv, nor = geo.attributes.normal;
  const P = [], U = [], N = [], S = [], W = [];
  let cut = 0;
  for (let t = 0; t < n; t += 3) {
    if (bad[t / 3]) { cut++; continue; }
    for (let k = 0; k < 3; k++) {
      const i = t + k;
      P.push(pos.getX(i), pos.getY(i), pos.getZ(i));
      N.push(nor ? nor.getX(i) : 0, nor ? nor.getY(i) : 0, nor ? nor.getZ(i) : 0);
      U.push(uv.getX(i), uv.getY(i));
      S.push(si.getX(i), si.getY(i), 0, 0);
      W.push(sw.getX(i), sw.getY(i), 0, 0);
    }
  }
  console.log('[demo] stretch cleanup removed', cut, 'triangles across', poseFns.length, 'poses');
  const out = new THREE.BufferGeometry();
  out.setAttribute('position', new THREE.Float32BufferAttribute(P, 3));
  out.setAttribute('normal', new THREE.Float32BufferAttribute(N, 3));
  out.setAttribute('uv', new THREE.Float32BufferAttribute(U, 2));
...
// CONSISTENT WINDING — AI-generated meshes ship triangles wound both ways, so a
// single-sided render culls the wrongly-wound half (looks gone) and a double-
// sided render can ghost on strict mobile GPUs. At this point the geometry still
// carries the GLB's ORIGINAL vertex normals (the intended outward direction), so
// any triangle whose winding-derived normal disagrees with them is flipped. After
// this, single-sided rendering is rock-solid on every device.
function fixWinding(geo) {
  const pos = geo.attributes.position, nor = geo.attributes.normal;
  const uv = geo.attributes.uv, si = geo.attributes.skinIndex, sw = geo.attributes.skinWeight;
  if (!nor) return;
  const A = new THREE.Vector3(), B = new THREE.Vector3(), C = new THREE.Vector3();
  const ab = new THREE.Vector3(), ac = new THREE.Vector3(), gn = new THREE.Vector3(), sn = new THREE.Vector3();
  const swap = (attr, i, j, comps) => {
    for (let c = 0; c < comps; c++) {
      const tmp = attr.array[i * comps + c];
      attr.array[i * comps + c] = attr.array[j * comps + c];
      attr.array[j * comps + c] = tmp;
    }
  };
  let flipped = 0;
  for (let t = 0; t < pos.count; t += 3) {
    ...
    sn.set(0, 0, 0);
    for (let k = 0; k < 3; k++) { sn.x += nor.getX(t + k); sn.y += nor.getY(t + k); sn.z += nor.getZ(t + k); }
    if (gn.dot(sn) < 0) {                          // winding disagrees with intended normals → flip
      swap(pos, t + 1, t + 2, 3); swap(nor, t + 1, t + 2, 3);
      if (uv) swap(uv, t + 1, t + 2, 2);
      if (si) swap(si, t + 1, t + 2, 4);
      if (sw) swap(sw, t + 1, t + 2, 4);
      flipped++;
    }
  }
  pos.needsUpdate = true; nor.needsUpdate = true;
  if (uv) uv.needsUpdate = true; if (si) si.needsUpdate = true; if (sw) sw.needsUpdate = true;
  console.log('[demo] winding fix flipped', flipped, 'of', pos.count / 3, 'triangles');
}
...
function buildSkinnedBody(kind, preset) {
  ...
  if (preset.meshyBody && MOLLY) {
    ...
    if (!MOLLY.bound) {
      ...
      // The creator nailed the diagnosis: his fingers were touching his thighs
      // when Meshy generated him, so the mesh FUSED finger-to-thigh — those welded
      // bridge triangles are what stretch when the arm raises. cutArmLegBridges
      // snips ONLY those ~80 bridge triangles (the lightest possible cut — one
      ...
      smoothSkinWeights(MOLLY.geo, 4);                 // diffuse weights — soft joints, no hard borders
      {
        ...
        [shL, elL, wrL, fiL, shR, elR, wrR, fiR].forEach(b => boneGroup[B.indexOf(b)] = 1);
        [hipL, knL, anL, toL, hipR, knR, anR, toR].forEach(b => boneGroup[B.indexOf(b)] = 2);
        clampWeightsToGroups(MOLLY.geo, boneGroup);    // stop arm weight bleeding onto the chest (the tear)
      }
    ...
    // Render config that LOADED FULLY on the creator's phone during the stretching
    // era: double-sided (the GLB's own setting) + opaque. Only the stray full-white
    // emissive is disabled (it can ghost on some GPUs and isn't part of his design).
    mMat.side = THREE.DoubleSide;
    mMat.transparent = false; mMat.opacity = 1; mMat.depthWrite = true; mMat.depthTest = true;
    mMat.emissiveIntensity = 0;
    if (mMat.emissive) mMat.emissive.setRGB(0, 0, 0);
    // ===== BUILD 78 STALE-FILE TEST: force Molotov BRIGHT GREEN. If the creator
    // sees green, new builds reach their screen; if he's still orange, they're
    // opening an old cached file. Revert once confirmed. =====
    if (mMat.color) mMat.color.setRGB(0.1, 1.0, 0.1);
    if (mMat.emissive) mMat.emissive.setRGB(0.0, 0.8, 0.0);
    mMat.emissiveIntensity = 1.0;
    mMat.map = null;                                    // drop the flame texture so green is unmistakable
...
function operatorModel(kind, name) {
  ...
  if (meshy) {
    ...
    return { grp, torso: spineB, bodyHit: bodyHit0, head: headHit0, headG: headB,
      armL: shL, armR: shR, legL: hipL, legR: hipR, gun: gun0, joints: joints0, lids: null, hold: HOLDS[preset.hold] || null, bones };
  }
  ...
  return { grp, torso: spineB, bodyHit, head: headHit, headG: headB,
    armL: shL, armR: shR, legL: hipL, legR: hipR, gun, joints, lids: builtHead.lids, hold: HOLDS[preset.hold] || null, bones };
}
...
// a clean RED-DOT optic for the player viewmodel — a see-through tube with a
// glowing dot floating inside; you aim by putting the dot on the target.
function playerRedDot(x, y, z) {
  const g = new THREE.Group();
  const dotMat = new THREE.MeshStandardMaterial({ color: 0xff2a15, emissive: 0xff1a08, emissiveIntensity: 4, roughness: 0.35 });
  const tube = new THREE.Mesh(new THREE.CylinderGeometry(0.016, 0.016, 0.052, 16, 1, true), gunDark);
  tube.rotation.x = Math.PI / 2;                                   // axis along the barrel (z)
  const rimF = new THREE.Mesh(new THREE.TorusGeometry(0.0165, 0.0026, 8, 18), gunDark); rimF.position.z = -0.027;
  const rimB = new THREE.Mesh(new THREE.TorusGeometry(0.0165, 0.0026, 8, 18), gunDark); rimB.position.z = 0.027;
  const dot = new THREE.Mesh(new THREE.SphereGeometry(0.0026, 8, 6), dotMat); dot.position.z = 0.008;
  const mount = part(gunDark, 0.02, 0.03, 0.03, 0, -0.024, 0);    // riser down to the rail
  g.add(tube, rimF, rimB, dot, mount);
  g.position.set(x, y, z);
  return g;
}
function buildM16() {
  ...
  g.add(playerRedDot(0, 0.098, -0.12));                                // RED DOT on the carry handle — clears the irons
  return { g, mag, bolt: chHandle, muzzle: new THREE.Vector3(0, 0, -0.78), pump: null,
    ads: new THREE.Vector3(0, -0.098, -0.34) };
}
function buildPP919() {
  const g = new THREE.Group();
  ...
  g.add(mk2);
  g.add(playerRedDot(0, 0.078, -0.2));                                 // RED DOT on the receiver
  return { g, mag: helical, bolt, muzzle: new THREE.Vector3(0, 0.012, -0.5), pump: null,
    ads: new THREE.Vector3(0, -0.078, -0.3) };
}
...
let bobPhase = 0;
let recoilRotX = 0, recoilPosZ = 0;   // last frame's recoil offset — removed before the sway lerp so it never accumulates
...
function updateWeapon(dt) {
  ...
  // undo last frame's recoil kick FIRST so the lerps operate on a clean base
  // (previously it fed back through the lerp and multiplied with framerate —
  // on a 240Hz screen that ran 4x, climbing the muzzle to the sky)
  rig.rotation.x -= recoilRotX; rig.position.z -= recoilPosZ;
  recoilRotX = 0; recoilPosZ = 0;
  ...
  // recoil spring — a CLEAN offset (no feedback, framerate-independent). Peak ~16deg
  // muzzle climb, recovering as the spring decays: visible to manage, never runaway.
  if (recoil > 0) {
    recoil = Math.max(0, recoil - dt * 7);
    const k = recoil * recoil;
    recoilPosZ = k * 0.04;
    recoilRotX = k * 0.28;
    rig.position.z += recoilPosZ;
    rig.rotation.x += recoilRotX;
  }
...
const BUILD = 78;   // bump with each demo update — shown on the badge so staleness is visible
