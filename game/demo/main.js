...
function operatorModel(kind, name) {
  ...
  if (combatant) {
    ...
    const shellGeo = new THREE.CylinderGeometry(0.315, 0.30, 0.30, 18, 1, true, -0.85, 1.7);
    const front = new THREE.Mesh(shellGeo, fab);
    front.scale.z = 0.62; front.rotation.y = Math.PI;                    // front face (-Z)
    front.position.set(0, -0.02, 0.015); chestB.add(front);
    const backShell = new THREE.Mesh(shellGeo, fab);
    backShell.scale.z = 0.66; backShell.position.set(0, -0.02, -0.015); chestB.add(backShell);
    // MOLLE webbing rows across the front shell
    for (let r2 = 0; r2 < 3; r2++) {
      const web = new THREE.Mesh(new THREE.CylinderGeometry(0.318, 0.318, 0.022, 18, 1, true, Math.PI - 0.72, 1.44), trim);
      web.scale.z = 0.62;
    ...
    for (let p = 0; p < 3; p++) {
      ...
      pouch.position.set(px, -0.135, -0.185); chestB.add(pouch);
      const flap = new THREE.Mesh(new THREE.CylinderGeometry(0.038, 0.038, 0.03, 8, 1, false), trim);
      flap.position.set(px, -0.095, -0.185); chestB.add(flap);
      const buckle = new THREE.Mesh(new THREE.SphereGeometry(0.008, 6, 5), buckleM);
      buckle.position.set(px, -0.11, -0.222); chestB.add(buckle);
    }
    // duty belt: elliptical band with a rounded buckle plate
    const belt = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.255, 0.055, 18, 1, true), trim);
    belt.scale.z = 0.66; belt.position.set(0, -0.045, 0); hipsB.add(belt);
    const bplate = new THREE.Mesh(new THREE.CapsuleGeometry(0.02, 0.03, 3, 8), buckleM);
    bplate.rotation.z = Math.PI / 2; bplate.position.set(0, -0.045, -0.165); hipsB.add(bplate);
    ...
    if (preset.tape) {
      ...
      tape.position.set(0, 0.09, -0.20); tape.rotation.y = Math.PI; chestB.add(tape);
    }
  }
...
const BUILD = 37;   // bump with each demo update — shown on the badge so staleness is visible
