...
const PRESETS = {
  'Molotov':         { face: { mask: 'molotov' }, hat: null, hairSide: '#8a2f10', blackTac: true, tape: 'MOLOTOV', meshyBody: true },
...
function bindGeoToBones(geo, B, segs) {
...
function buildSkinnedBody(kind, preset) {
  ...
  if (preset.meshyBody && MOLLY) {
    ...
    const mMat = new THREE.MeshStandardMaterial({ map: blackTacTex, roughness: 0.94 });
...
function operatorModel(kind, name) {
  ...
  if (meshy) {
    ...
    return { grp, torso: spineB, bodyHit: bodyHit0, head: headHit0, headG: headB,
      armL: shL, armR: shR, legL: hipL, legR: hipR, gun: gun0, joints: joints0, lids: null };
  }
...
const BUILD = 39;   // bump with each demo update — shown on the badge so staleness is visible
