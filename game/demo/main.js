...
function soldierModel(kind, name) {
  ...
  function makeArm(side) {                                   // side: -1 left, +1 right
    ...
    const hand = new THREE.Mesh(new THREE.BoxGeometry(0.105, 0.105, 0.105), handM);
  ...
  if (preset.tape) {                                         // "MOLOTOV" name tape on the carrier
    ...
    backing.position.set(0, 1.335, -0.135);
    const tape = markPlate([preset.tape], 0.15, 0.03, 'rgba(238,238,240,0.95)');
    tape.position.set(0, 1.335, -0.139);
...
const BUILD = 26;   // bump with each demo update — shown on the badge so staleness is visible
