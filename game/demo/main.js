...
function soldierModel(kind, name) {
  ...
  if (combatant) {                                           // READY: level gun, right hand grip / left hand barrel
    armRJ.shoulder.rotation.x = 0.25; armRJ.shoulder.rotation.z = -0.45; armRJ.elbow.rotation.x = 0.35;
    armLJ.shoulder.rotation.x = 0.95; armLJ.shoulder.rotation.z = 0.74;  armLJ.elbow.rotation.x = 0.15;
  ...
  if (isEnemy || kind === 'friendly') {
    ...
    gun.position.set(0.08, 1.14, -0.5);           // WHOLE gun forward of the body — butt at the shoulder, not through it
    gun.rotation.x = 0;
  ...
  for (const e of entities) {
    ...
    if (ai) {
      ...
      if (ai.reloadT > 0) {
        ...
        J[0].rotation.x = L(B(0).x + br, 1.28);          // left arm extends along the barrel
        J[0].rotation.z = L(B(0).z, 0.68);
        J[1].rotation.x = L(B(1).x, 0.05) - 0.2 * kick;
        if (e.model.gun) {
          const mg = e.model.gun.userData.mag;
          if (mg) mg.visible = true;
        }
      }
      // right arm: grip hand rear, elbow bends deeper as the gun rises
      J[2].rotation.x = L(B(2).x + br, 0.75) + 0.1 * kick;
      J[2].rotation.z = L(-0.45, -0.55);
      J[3].rotation.x = L(B(3).x, 1.8) - 0.25 * kick;
      ...
      if (e.model.gun) {
        ...
        g.position.x = L(g.userData.basePos.x, 0.05);
...
const BUILD = 14;   // bump with each demo update — shown on the badge so staleness is visible
