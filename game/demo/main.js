...
function buildSkinnedBody(kind, preset) {
  ...
  if (preset.meshyBody && MOLLY) {
    ...
    if (!MOLLY.bound) {
      ...
      // re-assert territories after smoothing (diffusion must not cross arm/leg)
      {
        const armIdx = new Set([shL, elL, wrL, fiL, shR, elR, wrR, fiR].map(b => B.indexOf(b)));
        const legIdx = new Set([hipL, knL, anL, toL, hipR, knR, anR, toR].map(b => B.indexOf(b)));
        const p2 = MOLLY.geo.attributes.position, si2 = MOLLY.geo.attributes.skinIndex, sw2 = MOLLY.geo.attributes.skinWeight;
        for (let i = 0; i < p2.count; i++) {
          const ax3 = Math.abs(p2.getX(i)), y3 = p2.getY(i);
          if (y3 > 1.0) continue;
          const ban = ax3 > 0.235 ? legIdx : ax3 < 0.205 ? armIdx : null;
          if (!ban) continue;
          let w0 = sw2.getX(i), w1b = sw2.getY(i);
          if (ban.has(si2.getX(i))) w0 = 0;
          if (ban.has(si2.getY(i))) w1b = 0;
          const tot = w0 + w1b;
          if (tot > 0) { sw2.setX(i, w0 / tot); sw2.setY(i, w1b / tot); }
        }
        sw2.needsUpdate = true;
        // FINAL SCISSORS: drop any triangle whose corners follow different limbs —
        // these are precisely (and only) the polygons that can stretch across the body
        const chainOf = (bi2) => armIdx.has(bi2) ? 1 : legIdx.has(bi2) ? 2 : 0;
        const P3 = [], U3 = [], N3 = [], S3 = [], W3 = [];
        const uv3 = MOLLY.geo.attributes.uv, no3 = MOLLY.geo.attributes.normal;
        let cut2 = 0;
        for (let t = 0; t < p2.count; t += 3) {
          const c1 = chainOf(si2.getX(t)), c2 = chainOf(si2.getX(t + 1)), c3 = chainOf(si2.getX(t + 2));
          const hasA = c1 === 1 || c2 === 1 || c3 === 1;
          const hasOther = c1 !== 1 || c2 !== 1 || c3 !== 1;
          // arms legitimately join the body ONLY at the shoulder — any arm-to-anything
          // span below shoulder height is fused geometry (fingertips to thigh OR coat)
          const topY = Math.max(p2.getY(t), p2.getY(t + 1), p2.getY(t + 2));
          if (hasA && hasOther && topY < 1.28) { cut2++; continue; }
          for (let k = 0; k < 3; k++) {
            P3.push(p2.getX(t + k), p2.getY(t + k), p2.getZ(t + k));
            N3.push(no3.getX(t + k), no3.getY(t + k), no3.getZ(t + k));
            U3.push(uv3.getX(t + k), uv3.getY(t + k));
            S3.push(si2.getX(t + k), si2.getY(t + k), 0, 0);
            W3.push(sw2.getX(t + k), sw2.getY(t + k), 0, 0);
          }
        }
        console.log('[demo] cross-limb scissors removed', cut2, 'triangles');
        const g3 = new THREE.BufferGeometry();
        g3.setAttribute('position', new THREE.Float32BufferAttribute(P3, 3));
        g3.setAttribute('normal', new THREE.Float32BufferAttribute(N3, 3));
        g3.setAttribute('uv', new THREE.Float32BufferAttribute(U3, 2));
        g3.setAttribute('skinIndex', new THREE.Uint16BufferAttribute(S3, 4));
        g3.setAttribute('skinWeight', new THREE.Float32BufferAttribute(W3, 4));
        MOLLY.geo = g3;
      }
...
const BUILD = 65;   // bump with each demo update — shown on the badge so staleness is visible
