...
function soldierModel(kind) {
  ...
  const head = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.26, 0.24),
    kind === 'friendly' ? new THREE.MeshStandardMaterial({ color: 0x181818, roughness: 0.9 }) : skinMat); // Molotov: ski mask
  head.position.y = 1.66;
  // helmet for combatants
  if (isEnemy) {
    const helmet = new THREE.Mesh(new THREE.SphereGeometry(0.16, 10, 6, 0, Math.PI * 2, 0, 1.5), suit);
    helmet.position.y = 1.78; grp.add(helmet);
  }
  if (kind === 'friendly') { // fire decal stripe on Molotov's mask
  ...
  // rifle prop for combatants
  if (isEnemy || kind === 'friendly') {
    const gun = new THREE.Mesh(new THREE.BoxGeometry(0.07, 0.1, 0.62),
      new THREE.MeshStandardMaterial({ map: metalTex, roughness: 0.5, metalness: 0.5 }));
    gun.position.set(0.12, 1.12, -0.28); gun.rotation.x = -0.12;
    grp.add(gun);
  }
  return { grp, torso, head, armL, armR, legL, legR };
}
function makeEntity(name, kind, x, z) {
  const colors = { enemy: 0xf38ba8, protected: 0xf38ba8, friendly: 0x89b4fa, neutral: 0xe6e6e6 };
  const m = soldierModel(kind);
  m.grp.position.set(x, 0, z);
  scene.add(m.grp);
  const e = { name, kind, grp: m.grp, body: m.torso, head: m.head, model: m, color: colors[kind],
    hp: 100, alive: true, dying: 0, baseX: x, baseZ: z, phase: Math.random() * 9, shootTimer: 1 + Math.random() * 2.5,
    muzzleT: 0 };
  entities.push(e);
  return e;
}
...
function buildM16() {
  ...
  g.add(upper, handguard, barrel, carry, frontSight, mag, grip, stock);
  return { g, mag, muzzle: new THREE.Vector3(0, 0, -0.97), pump: null };
}
function buildPP919() {
  const g = new THREE.Group();
  ...
  g.add(body, shroud, helical, grip, stock);
  return { g, mag: helical, muzzle: new THREE.Vector3(0, 0.01, -0.56), pump: null };
}
function buildBenelli() {
  ...
  return { g, mag: null, muzzle: new THREE.Vector3(0, 0.02, -0.78), pump };
}
function buildStatesman() {
  ...
  g.add(slide, frame, grip, trigger, sight);
  return { g, mag: grip, muzzle: new THREE.Vector3(0, 0, -0.24), pump: slide };
}
...
function updateWeapon(dt) {
  const s = curS();
  const target = aiming && !s.reloading ? ADS : HIP;
...
function updateEntities(dt) {
  for (const e of entities) {
    ...
    e.grp.position.y = Math.sin(e.phase * 2) * 0.03;
