...
function soldierModel(kind, name) {
  ...
  const head = new THREE.Group();                            // skull + curved face panel
  const skull = new THREE.Mesh(new THREE.SphereGeometry(0.135, 14, 11), sideM);
  skull.scale.set(0.95, 1.12, 1.0);
  skull.castShadow = true;
  // the face wraps a curved panel on the front of the skull — no more flat box
  ...
  return { grp, torso, head: head.children[0], headG: head, armL, armR, legL, legR, gun, joints };
}
