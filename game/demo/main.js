...
function buildSkinnedBody(kind, preset) {
  ...
  if (preset.meshyBody && MOLLY) {
    ...
    if (!MOLLY.bound) {
      ...
      // build a bone-index -> group map (0 torso / 1 arm / 2 leg) and CLAMP: smoothing
      // may not move any vertex's influence into a different limb than it started in.
      // This kills the chest-centre tear (arm weight bleeding onto the sternum) that
      // per-triangle stretch detection structurally cannot catch — the patch moves
      // rigidly with the arm, so it only tears at the SEAM with its neighbours.
      {
        ...
        clampWeightsToGroups(MOLLY.geo, boneGroup);
      }
      // DEFINITIVE, CAUSE-AGNOSTIC cleanup: pose the skeleton through the real
      // animation extremes (shouldered aim, both kneels, reload cross-draw) and
      // delete any triangle that physically TEARS. Whatever the fusion — fingertip
      // to thigh, fingertip to coat, sleeve web — it can only surface as a triangle
      // that stretches when the limbs move, so this removes all of them at once
      // while sparing real joint creases (which stay small even when they spike).
      MOLLY.geo = deleteStretchTriangles(MOLLY.geo, B, [
        () => { shL.rotation.set(1.5, 0.42, 0.5); elL.rotation.x = 1.35;
                shR.rotation.set(1.5, -0.42, -0.5); elR.rotation.x = 1.35; },                                    // shouldered aim
        () => { shL.rotation.set(1.5, 0.42, 0.5); elL.rotation.x = 1.35;
                shR.rotation.set(1.5, -0.42, -0.5); elR.rotation.x = 1.35;
                hipL.rotation.x = 1.4; knL.rotation.x = -1.72; hipR.rotation.x = -0.4; knR.rotation.x = -1.72; }, // aim + right-knee kneel
        () => { shL.rotation.set(1.5, 0.42, 0.5); elL.rotation.x = 1.35;
                shR.rotation.set(1.5, -0.42, -0.5); elR.rotation.x = 1.35;
                hipR.rotation.x = 1.4; knR.rotation.x = -1.72; hipL.rotation.x = -0.4; knL.rotation.x = -1.72; }, // aim + left-knee kneel
        () => { shL.rotation.set(0.9, 0.2, 0.6); elL.rotation.x = 1.7;
                shR.rotation.set(0.4, -0.2, -0.3); elR.rotation.x = 0.6; },                                      // reload / patrol cross-draw
      ], 2.2, 0.09);
      MOLLY.geo = removeSmallIslands(MOLLY.geo, 250);
      fixWinding(MOLLY.geo);            // orient every face OUTWARD (AI meshes ship mixed winding)
    ...
    // The GLB ships doubleSided + full-white emissive — the combo renders SOLID in
    // desktop GL but ghostly/see-through on strict mobile (iOS) GPUs. With winding
    // now consistent we can render single-sided (rock-solid everywhere), and the
    // stray white self-illumination is killed so he can't glow into transparency.
    mMat.side = THREE.FrontSide;
...
const BUILD = 70;   // bump with each demo update — shown on the badge so staleness is visible
