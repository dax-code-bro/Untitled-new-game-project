...
const B9_BUILD = { version: '0.19.0', name: 'the floor knows what is standing on it' };
...
/* MOVED INTO THE ENGINE. Multiplayer needs the same surface to close a
   hand onto a weapon, and a copy in each game is how the two viewmodels
   drifted far enough apart that one of them drew no gun at all. */
function weaponSurface(game, root) {
  return game.weaponSurface ? game.weaponSurface(root) : null;
}
...
function updateZombie(game, S, P, z, dt, sfx) {
  ...
  /* THE SHAMBLE PLAYS AT THE SPEED THE SHAMBLE TRAVELS. clipSpeed is a
     constant per variant, and a constant cannot match a body whose own
     speed is rolled between 0.95 and 1.55 and then multiplied by a wave
     boost -- so every zombie's feet slid on the floor by whatever the
     difference happened to be. The shamble clips now state how far one
     cycle carries the body (AnimationClip.stride, generated by
     engine/tools), so the rate that makes a planted foot planted is
     simply the body's speed over the clip's own. clipSpeed stays, as
     the per-variant lean on top of it: a heavy one drags, a lucid one
     hurries. */
  if (z.actor.animator) {
    const cyc = z.actor.animator.clips.get(z.anim);
    const v = Math.hypot(z.actor.controller.body.velocity.x, z.actor.controller.body.velocity.z);
    const base = (cyc && cyc.stride && v > 0.2) ? window.LE.gaitRate(cyc, v, 0.6, 1.7) : 1;
    z.actor.animator.speed = base * V.clipSpeed * (z.lucid ? 1.05 : 1);
  }
