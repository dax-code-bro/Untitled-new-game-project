...
void World::syncPlaced(const Sim& sim) {
    ...
    for (const Placed& p : sim.placed) {
        ...
        if (p.kind == BuildKind::DogRun) {
            ...
            AABB a = p.bounds(gy);
            if (p.kind == BuildKind::Tree || p.kind == BuildKind::LampPost || p.kind == BuildKind::SecurityCamera) {
                vec3 c = a.center();
                float r = p.kind == BuildKind::Tree ? 0.35f : 0.12f;
